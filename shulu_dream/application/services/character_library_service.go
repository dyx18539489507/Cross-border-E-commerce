...
func (s *CharacterLibraryService) ListLibraryItems(query *CharacterLibraryQuery, deviceID string) ([]models.CharacterLibrary, int64, error) {
	var items []models.CharacterLibrary
	var total int64

	db := s.db.Model(&models.CharacterLibrary{}).Where("device_id = ?", deviceID)
...
func (s *CharacterLibraryService) CreateLibraryItem(req *CreateLibraryItemRequest, deviceID string) (*models.CharacterLibrary, error) {
...
func (s *CharacterLibraryService) GetLibraryItem(itemID string, deviceID string) (*models.CharacterLibrary, error) {
	var item models.CharacterLibrary
	err := s.db.Where("id = ? AND device_id = ?", itemID, deviceID).First(&item).Error
...
func (s *CharacterLibraryService) DeleteLibraryItem(itemID string, deviceID string) error {
	result := s.db.Where("id = ? AND device_id = ?", itemID, deviceID).Delete(&models.CharacterLibrary{})
...
func (s *CharacterLibraryService) ApplyLibraryItemToCharacter(characterID string, libraryItemID string, deviceID string) error {
	// 验证角色库项存在且属于该用户
	var libraryItem models.CharacterLibrary
	if err := s.db.Where("id = ? AND device_id = ?", libraryItemID, deviceID).First(&libraryItem).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return errors.New("library item not found")
		}
		return err
	...
	if err := s.db.Joins("JOIN dramas ON dramas.id = characters.drama_id").
		Where("characters.id = ? AND dramas.device_id = ?", characterID, deviceID).
...
func (s *CharacterLibraryService) UploadCharacterImage(characterID string, imageURL string, deviceID string) error {
	// 查找角色
	var character models.Character
	if err := s.db.Joins("JOIN dramas ON dramas.id = characters.drama_id").
		Where("characters.id = ? AND dramas.device_id = ?", characterID, deviceID).
...
func (s *CharacterLibraryService) AddCharacterToLibrary(characterID string, category *string, deviceID string) (*models.CharacterLibrary, error) {
	// 查找角色
	var character models.Character
	if err := s.db.Joins("JOIN dramas ON dramas.id = characters.drama_id").
		Where("characters.id = ? AND dramas.device_id = ?", characterID, deviceID).
...
func (s *CharacterLibraryService) DeleteCharacter(characterID uint, deviceID string) error {
	// 查找角色
	var character models.Character
	if err := s.db.Joins("JOIN dramas ON dramas.id = characters.drama_id").
		Where("characters.id = ? AND dramas.device_id = ?", characterID, deviceID).
...
func (s *CharacterLibraryService) GenerateCharacterImage(characterID string, imageService *ImageGenerationService, modelName string, deviceID string) (*models.ImageGeneration, error) {
	// 查找角色
	var character models.Character
	if err := s.db.Joins("JOIN dramas ON dramas.id = characters.drama_id").
		Where("characters.id = ? AND dramas.device_id = ?", characterID, deviceID).
	...
	imageGen, err := imageService.GenerateImage(req, deviceID)
	if err != nil {
...
func (s *CharacterLibraryService) UpdateCharacter(characterID string, req interface{}, deviceID string) error {
	// 查找角色
	var character models.Character
	if err := s.db.Joins("JOIN dramas ON dramas.id = characters.drama_id").
		Where("characters.id = ? AND dramas.device_id = ?", characterID, deviceID).
...
func (s *CharacterLibraryService) BatchGenerateCharacterImages(characterIDs []string, imageService *ImageGenerationService, modelName string, deviceID string) {
	...
	for _, characterID := range characterIDs {
		...
		go func(charID string) {
			imageGen, err := s.GenerateCharacterImage(charID, imageService, modelName, deviceID)
			if err != nil {
