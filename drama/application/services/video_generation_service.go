...
func (s *VideoGenerationService) getVideoClient(provider string, modelName string) (video.VideoClient, error) {
	...
	case "doubao", "volcengine", "volces":
		endpoint = normalizeVolcesEndpoint(baseURL, strings.TrimSpace(config.Endpoint), "/api/v3/contents/generations/tasks")
		queryEndpoint = normalizeVolcesEndpoint(baseURL, strings.TrimSpace(config.QueryEndpoint), "/api/v3/contents/generations/tasks/{taskId}")
