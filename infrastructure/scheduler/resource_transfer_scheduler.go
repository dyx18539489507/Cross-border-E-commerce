...
func (s *ResourceTransferScheduler) transferPendingResources() {
	s.log.Info("Resource transfer scheduler skipped: current build no longer exposes MinIO batch transfer methods")
}

// transferAllPendingResources 转存所有待转存的资源（全量扫描）
func (s *ResourceTransferScheduler) transferAllPendingResources() {
	s.log.Info("Full resource transfer scan skipped: current build no longer exposes MinIO batch transfer methods")
}
