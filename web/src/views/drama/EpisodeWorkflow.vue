    ...
    >
        ...
        <el-form-item label="动作描述（可选）">
          <el-input
            ...
            type="textarea"
            :autosize="{ minRows: 2, maxRows: 3 }"
            placeholder="添加动作描述和镜头语言，如人物走向镜头，背景虚化平视跟拍"
            class="digital-human-textarea"
          />
        </el-form-item>
      </el-form>
...
const parseCustomVoiceID = (voiceID: string): string => {
  const raw = (voiceID || '').trim()
  if (!raw.startsWith('custom-')) return ''
  return raw.slice('custom-'.length)
}

const applyFallbackAudioUpload = (file: File) => {
...
const pollCustomVoiceReady = async (voiceID: string): Promise<VoiceLibraryItem> => {
  const maxAttempts = 40
  const interval = 3000

  for (let index = 0; index < maxAttempts; index++) {
    await new Promise(resolve => setTimeout(resolve, interval))
    const latest = await voiceLibraryAPI.getCustomStatus(voiceID)
    ...
    if (status === 'failed') {
      throw new Error(latest.last_error || '音色训练失败')
    }
  }

  throw new Error('音色训练超时，请稍后在音色库中查看状态')
}
...
const handleUploadVoiceFromCard = async (event: Event) => {
  ...
  try {
    ElMessage.info('正在创建音色，请稍候...')
    const baseName = file.name.replace(/\.[^/.]+$/, '').trim()
    const created = await voiceLibraryAPI.createCustom(file, baseName || undefined)

    let finalVoice = created
    const customID = parseCustomVoiceID(created.id)
    if (customID && (created.status || '').toLowerCase() !== 'completed') {
      ElMessage.info('音色训练中，正在自动刷新状态...')
      finalVoice = await pollCustomVoiceReady(customID)
    }

    await loadVoiceLibrary()
    const matchedVoice = voiceLibraryList.value.find(item => item.id === finalVoice.id)
    selectedVoice.value = matchedVoice || finalVoice
    clearUploadedAudioState()
    voiceLibraryVisible.value = false
    ElMessage.success('音色创建成功，已加入音色库')
  } catch (error: any) {
    const message = String(error?.message || '')
    applyFallbackAudioUpload(file)
    ElMessage.warning(message ? `音色创建失败，已切换为音频驱动模式：${message}` : '音色创建失败，已切换为音频驱动模式')
  } finally {
