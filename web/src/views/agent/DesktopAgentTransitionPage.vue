      ...
      <header class="desktop-agent-header">
        <div class="desktop-agent-header__left">
          ...
          <div class="desktop-agent-header__copy">
            ...
            <p>DeepSeek 正在理解商品信息、目标市场、合规边界与营销目标。</p>
          </div>
        </div>
...
const inferRecognizedInfo = (input: AgentInput | null, prompt: string): RecognizedInfo => {
  const product = input?.productName || extractMatch(prompt, /(?:我有|我们有|这是一款|这款|一款|一个|一种|商品是|产品是)([^，,。；;\n]{2,28}?)(?:，|,|。|；|;|想|计划|准备|主打|目标|卖到|出口|做|$)/) || '便携榨汁杯'
  const category = input?.category || (product.includes('榨汁杯') ? '小家电 / 食品接触用品' : '跨境电商商品')
  const market = input?.targetMarket || extractMatch(prompt, /(?:卖到|出口到|进入|面向|投放到|推广到)([^，,。；;\n]{2,24}?)(?:市场|用户|消费者|，|,|。|；|;|$)/) || '马来西亚'
...
const buildFallbackResult = (input: AgentInput, info: RecognizedInfo): AgentResult => {
  ...
  return {
    ...
    localization: {
      ...
      reason: '马来西亚 TikTok 用户更容易被真实生活场景、轻量健康习惯和清晰产品演示吸引。',
      keywords: ['portable', 'daily juice', 'easy clean', 'office lifestyle'],
      tone: '自然、可信、少夸张',
    ...
    script: {
      ...
      middle: { time: '3-20s', content: '展示便携榨汁杯的外观、使用步骤和清洗方式，强调日常场景价值，避免绝对化承诺。' },
    ...
    digitalHuman: {
      ...
      tone: '自然、可靠、少夸张',
      videoRatio: '9:16',
      subtitleAdvice: '中文脚本 + 英文/马来语重点字幕',
