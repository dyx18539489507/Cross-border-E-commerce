          ...
          <div class="track-clips">
            ...
            >
              <div class="clip-content">
                ...
                <div class="clip-info">
                  <div class="clip-title">{{ audio.title || ($t('video.audio') + ' ' + (audio.order + 1)) }}</div>
...
const addAudioClipFromAsset = async (asset: { id?: number | string; url: string; duration?: number; name?: string }, insertAtPosition?: number) => {
  if (!asset?.url) {
    ElMessage.warning('音频地址不能为空')
    return
  }

  let audioDuration = asset.duration || 0
  if (!audioDuration || audioDuration <= 0) {
    try {
      audioDuration = await getAudioDuration(asset.url)
    } catch (error) {
      console.warn('Failed to get audio duration, using fallback duration:', error)
      audioDuration = Math.max(totalDuration.value || 0, 5)
    }
  }

  const clipPosition = Math.max(0, insertAtPosition ?? currentTime.value)

  const newClip: AudioClip = {
    id: `audio_asset_${Date.now()}_${asset.id ?? 'custom'}`,
    source_clip_id: asset.id !== undefined ? `asset_${asset.id}` : `external_${Date.now()}`,
    audio_url: asset.url,
    start_time: 0,
    end_time: audioDuration,
    duration: audioDuration,
    position: clipPosition,
    order: audioClips.value.length,
    volume: 1,
    title: asset.name
  }
  ...
  selectedAudioClipId.value = newClip.id

  ElMessage.success('已添加音频到时间线')
}
